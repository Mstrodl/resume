...
\begin{document}
\name{Galen Guyer}
\tagline{Full Stack Software Engineer Seeking a Summer 2021 Co-op}
\personalinfo{%
...
\begin{itemize}
\item Consists of seven Azure Virtual Machines distributed around the globe to provide the fastest server to the user based on geographical location
\item Uses shell scripts and rsync to distribute static files, configuration files, and SSL certificates from a central server to all edge servers
\item Averages sub-100ms latency around the globe for my personal website and CDN, beating Cloudflare's speeds for a cache miss and being comparable for cache hits
\end{itemize}
